...
class AdmPadHisResourceIT {
    ...
    void createAdmPadHis() throws Exception {
        ...
        AdmPadHisDTO admPadHisDTO = admPadHisMapper.toDto(admPadHis);
        restAdmPadHisMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(admPadHisDTO)))
    ...
    void putNewAdmPadHis() throws Exception {
        ...
        AdmPadHisDTO admPadHisDTO = admPadHisMapper.toDto(updatedAdmPadHis);

        restAdmPadHisMockMvc
            .perform(
                put(ENTITY_API_URL_ID, admPadHisDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(admPadHisDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAdmPadHis() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAdmPadHisMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(admPadHisDTO)))
    ...
    void partialUpdateAdmPadHisWithPatch() throws Exception {
        ...
        partialUpdatedAdmPadHis.userId(UPDATED_USER_ID).createTime(UPDATED_CREATE_TIME);
    ...
    void patchNonExistingAdmPadHis() throws Exception {
        ...
        restAdmPadHisMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, admPadHisDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(admPadHisDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAdmPadHis() throws Exception {
        ...
        restAdmPadHisMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(admPadHisDTO))
            )
            .andExpect(status().isMethodNotAllowed());
