...
public class AdmFileResource {
    ...
    public AdmFileResource(AdmFileService admFileService, AdmFileRepository admFileRepository) {
    ...
    public ResponseEntity<Void> deleteAdmFile(@PathVariable Long id) throws IOException {
        log.debug("REST request to delete AdmFile : {}", id);

        admFileService.delete(id);
        return ResponseEntity
            .noContent()
    ...
    @PostMapping("/adm-files/pic")
    public byte[] getPicFile(@RequestBody AdmFile admFile) throws URISyntaxException, IOException {
        log.debug("REST request to get pic : {}", admFile);

        Optional<AdmFile> picFileInfo = admFileRepository.findByModuleTypeAndBusinessTypeAndSourceIdOrderBySourceId(
            admFile.getModuleType(), admFile.getBusinessType(), admFile.getSourceId()
        );

        if(picFileInfo.isPresent()) {
            Path path = Paths.get(picFileInfo.get().getS3Url());
            if(path.toFile().exists()) {
                return Files.readAllBytes(Paths.get(picFileInfo.get().getS3Url()));
            }else {
    ...
    @GetMapping("/adm-files/download/{id}")
    public DownloadableResource getFile(@PathVariable Long id) throws IOException {
        log.debug("REST request to get File : {}", id);
        Optional<AdmFile> admFile = admFileRepository.findById(id);
        if(admFile.isPresent()) {
            Path path = Path.of(admFile.get().getS3Url());
            if(path.toFile().exists()) {
                return new DownloadableResource(new ByteArrayResource(Files.readAllBytes(path)), admFile.get().getFileNameOri());
            }else {
    ...
    @GetMapping("/service/adm-files/download/{id}")
    public DownloadableResource downloadFile(@PathVariable String id) throws IOException {
        log.debug("REST request to get File : {}", id);
        Optional<AdmFile> admFile = admFileRepository.findById(Long.valueOf(id));
        if(admFile.isPresent()) {
            Path path = Path.of(admFile.get().getS3Url());
            if(path.toFile().exists()) {
                return new DownloadableResource(new ByteArrayResource(Files.readAllBytes(path)), path.toFile().getName());
            }else {
                admFileRepository.delete(admFile.get());
                return null;
            }
