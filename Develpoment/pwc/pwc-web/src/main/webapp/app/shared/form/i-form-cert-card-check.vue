<template>
  <div class="i-form-cert-card-check-wrapper">
    <div class="" v-if="isPIDModelProp">
      <div class="d-flex align-items-center justify-content-center mb-4">
        <b-col cols="2" class="text-left">
          <b-form-select
            id="slotDescription"
            :options="slotOptions"
            v-model="vDataProp.slotDescription.$model"
            :disabled="disabled"
          ></b-form-select>
        </b-col>
        <b-col cols="2">
          <b-button class="mr-1" variant="info" @click.stop="reMyLoad" :disabled="reReadCardDisabled" v-if="slotOptions.length === 0"
            ><b-icon icon="credit-card"></b-icon>重新讀卡</b-button
          >
          <b-button class="mr-1" variant="success" @click.stop="startTest" :disabled="disabled" v-else
            ><b-icon icon="credit-card"></b-icon>讀卡成功</b-button
          >
          <!-- <i-button class="mr-1" variant="success" type="credit-card" @click.stop="startTest" :disabled="disabled" v-else></i-button> -->
        </b-col>
      </div>
      <div class="d-flex align-items-center justify-content-center mb-4">
        <b-col cols="2" class="text-right"><span>身分證統一編號：</span></b-col>
        <b-col cols="2" class="text-left" v-if="vDataProp.pID">
          <b-form-input
            cols="4"
            v-model="vDataProp.pID.$model"
            type="text"
            :disabled="disabled"
            placeholder="身分證統一編號"
          ></b-form-input>
      ...
      <div class="d-flex align-items-center justify-content-center mb-4">
        <b-col cols="2" class="text-right"><span>PIN碼：</span></b-col>
        <b-col cols="2" class="text-left" v-if="vDataProp.pID">
          <b-form-input
            cols="4"
            v-model="vDataProp.smartCard.$model"
            type="password"
            :disabled="disabled"
            placeholder="PIN碼"
          ></b-form-input>
    ...
    <div class="display_center_start" v-else>
      <b-col cols="4" class="text-left">
        <b-form-radio v-if="cardInfoProp.radioYStr && cardInfoProp.radioYStr !== ''" value="Y" v-model="vDataProp.agencyCardType.$model">{{
          cardInfoProp.radioYStr
        }}</b-form-radio>
        <b-form-radio v-else value="Y" v-model="vDataProp.agencyCardType.$model">(是) 插入卡片並輸入序號</b-form-radio>
      </b-col>
      ...
      <b-col cols="3" class="text-left" v-if="vDataProp.smartCard">
        <b-input-group>
          <template #append>
          ...
          <b-form-input
            cols="4"
            v-model="vDataProp.smartCard.$model"
            :type="showPwdNew2 ? 'password' : 'text'"
            :disabled="disabled"
            :placeholder="isPIDModelProp ? 'PIN碼' : ''"
            trim
          ></b-form-input>
        </b-input-group>
      </b-col>
      <b-col cols="3">
        <b-button class="mr-1" variant="info" @click.stop="reMyLoad" :disabled="reReadCardDisabled" v-if="slotOptions.length === 0"
          ><b-icon icon="credit-card"></b-icon>重新讀卡</b-button
        >
        <i-button class="mr-1" type="credit-card" @click.stop="startTest" :disabled="disabled" v-else></i-button>
      </b-col>
    </div>
    <div class="display_center_start" v-if="!isPIDModelProp">
      <b-col>
        <b-link cols="3" href="http://localhost:61161/selfTest.htm" target="_blank">　 使用環境建議與檢測</b-link>
        <b-link cols="3" href="https://hackmd.io/pDBH0NSvTH6p0HOWEzpW8w?view" target="_blank">　 介面使用操作及簡易問題排除</b-link>
      </b-col>
    </div>
    <b-col cols="12  pt-2" class="text-left" v-if="!isPIDModelProp">
      <b-form-radio v-if="cardInfoProp.radioNStr && cardInfoProp.radioNStr !== ''" value="N" v-model="vDataProp.agencyCardType.$model">{{
        cardInfoProp.radioNStr
      }}</b-form-radio>
      <b-form-radio v-else value="N" v-model="vDataProp.agencyCardType.$model"
...
import { computed, defineComponent, onUpdated, reactive, ref, onMounted, onUnmounted } from '@vue/composition-api';
import { BButton, BCol, BFormInput, BFormRadio, BFormSelect, BIcon, BLink, BInputGroup, BInputGroupText } from 'bootstrap-vue';
import {
...
export default defineComponent({
  name: 'IFormCertCardCheck',
  components: { IButton, BButton, BCol, BFormInput, BFormRadio, BFormSelect, BLink, BIcon, BInputGroup, BInputGroupText },
  props: {
    isPIDModel: { required: false, type: Boolean, default: false },
    cardInfo: { required: true, type: Object, default: () => {} },
    vData: { required: true, type: Object, default: () => {} },
  },
  setup(props, context) {
    const isPIDModelProp = ref(props.isPIDModel);
    const cardInfoProp = reactive(props.cardInfo);
    const vDataProp = reactive(props.vData);
    const $bvModal = useBvModal();
    const alreadyGetPKCS11Info = ref(false);
    const alreadyMyLoad = ref(false);
    const alreadyAddEventListener = ref(false);
    const slotOptions = ref([]);
    const setUserCertOK = ref(false);
    const setSignatureOK = ref(false);

    let signTarget, pkcsTarget, signTargetId, pkcsTargetId, imageInfo;

    let showPwdNew2 = true;

    const disabled = computed(() => {
      if (isPIDModelProp.value) return false;
      return vDataProp.agencyCardType.$model === 'N' || _size(slotOptions.value) === 0;
    });
    const reReadCardDisabled = computed(() => {
      if (isPIDModelProp.value) return false;
      return vDataProp.agencyCardType.$model === 'N';
    });
    ...
    function reMyLoad() {
      alreadyMyLoad.value = false;
      myLoad();
    }

    //myLoad 將晶片資料載入及取得讀卡機
    function myLoad() {
      if (alreadyMyLoad.value) return;
      else alreadyMyLoad.value = true;
      console.log('myLoad');
      let img = null,
        ctx,
        output = '',
    ...
    //將憑證資訊送出
    function setOutput(imageInfo) {
      const ret = JSON.parse(imageInfo);
      if (ret.ret_code == 0x76000031) {
        alert(window.location.hostname + '非信任網站，請先加入信任網站');
        window.history.back();
        return;
      }
      if (ret.ret_code === 1979711501) return;
      slotOptions.value = [];
      if (!_isArray(ret.slots) || _size(ret.slots) === 0) {
        vDataProp.slotDescription.$model = '';
        $bvModal.msgBoxOk('未檢測到卡片');
      } else {
        _forEach(ret.slots, s => {
          if (_includes(_keys(s), 'token') && _isObject(s.token))
            slotOptions.value.push({ value: s.slotDescription, text: s.slotDescription });
        });
        vDataProp.slotDescription.$model = _head(slotOptions.value).value;
      }
    }

    //呼叫驗證程式
    function openSignForm() {
      signTarget = window.open('http://localhost:61161/popupForm', '簽章中', 'height=200, width=200, left=0, top=0');
      pkcsTargetId = setTimeout(() => {
        checkFinish(signTarget);
      }, 60000);
    }

    function openPKCS11InfoForm() {
      pkcsTarget = window.open('http://localhost:61161/popupForm', '憑證讀取中', 'height=200, width=200, left=0, top=0');
      pkcsTargetId = setTimeout(() => {
        checkFinish(pkcsTarget);
      }, 60000);
    }
    ...
    function getTbsPackage() {
      ...
      tbsData['pin'] = vDataProp.smartCard.$model;
      tbsData['nonce'] = '';
    ...
    function getPKCS11InfoPackage() {
      const obj = {};
      obj['func'] = 'GetUserCert';
      const json = JSON.stringify(obj).replace(/\+/g, '%2B');
      return json;
    }

    //處理錯誤訊息
    function setUserCert(certData) {
      console.log('setUserCert');
      const ret = JSON.parse(certData);
      let errorMsg = '讀取憑證錯誤';
      if (ret.ret_code != 0) {
        errorMsg = MajorErrorReason(ret.ret_code);
        if (ret.last_error) errorMsg = errorMsg + '，' + MinorErrorReason(ret.last_error);
        $bvModal.msgBoxOk(errorMsg);
      } else {
        cardInfoProp.ori_pkcs11info = _cloneDeep(ret);
        openSignForm();
        if (!alreadyGetPKCS11Info.value) alreadyGetPKCS11Info.value = !alreadyGetPKCS11Info.value;
      }
      afterDoCheck('setUserCert');
    }
    ...
    function setSignature(signature) {
      ...
      if (ret.ret_code != 0) {
        ...
        cardInfoProp.ori_signResult = _cloneDeep(ret);
        if (alreadyGetPKCS11Info.value) alreadyGetPKCS11Info.value = !alreadyGetPKCS11Info.value;
      }
      afterDoCheck('setSignature');
    }

    function afterDoCheck(condition) {
      if (condition === 'setUserCert') setUserCertOK.value = true;
      if (condition === 'setSignature') setSignatureOK.value = true;
      if (setUserCertOK.value && setSignatureOK.value) {
        console.log('setUserCertOK and setSignatureOK');
        context.emit('afterStartTest');
        setUserCertOK.value = false;
        setSignatureOK.value = false;
      }
    }
    ...
    function startTest() {
      //輸入pin碼檢查
      if (_includes(_keys(vDataProp), 'pID')) vDataProp.pID.$model = _upperFirst(vDataProp.pID.$model);
      const pin = vDataProp.smartCard.$model;
      ...
      //檢查HiCOS版本
      const HiCOSValidObj = checkHiCOSVersion(imageInfo);
      if (!HiCOSValidObj.valid && HiCOSValidObj.msg !== '') {
        $bvModal.msgBoxOk(HiCOSValidObj.msg);
        return;
      }
      //開啟驗證通道
      openPKCS11InfoForm();
    ...
    function receiveMessage(event) {
      ...
      try {
        ...
        if (ret.func) {
          if (ret.func == 'getTbs') {
            console.log('getTbs');
            if (!alreadyGetPKCS11Info.value) {
              console.log('alreadyGetPKCS11Info');
              clearTimeout(pkcsTargetId);
              pkcsTarget.postMessage(getPKCS11InfoPackage(), '*');
            } else {
              console.log('signTargetId');
              clearTimeout(signTargetId);
              signTarget.postMessage(getTbsPackage(), '*');
            }
          } else if (ret.func == 'sign') {
            setSignature(event.data);
          } else if (ret.func == 'pkcs11info') {
            setUserCert(event.data);
          } else alert('unknown func ' + ret.func);
        } else {
    ...
    function checkFinish(target) {
      if (target) target.close();
      // $bvModal.msgBoxOk('請檢查瀏覽器設定');
    ...
    function checkHiCOSVersion(info) {
      let validObj = { valid: true, msg: '' };
      const ret = JSON.parse(info),
        nVer = window.navigator.appVersion,
        version = ret.serverVersion;
      if (nVer.indexOf('Win') >= 0) {
        if (version < '1.3.4.103328') {
          validObj.valid = false;
          validObj.msg = '目前的版本為' + version + '，請升級至1.3.4.103328';
        }
      } else if (nVer.indexOf('Mac') >= 0) {
        if (version < '1.3.4.13') {
          validObj.valid = false;
          validObj.msg = '目前的版本為' + version + '，請升級至1.3.4.13';
        }
      } else if (nVer.indexOf('Linux') >= 0) {
        if (version < '1.3.4.3') {
          validObj.valid = false;
          validObj.msg = '目前的版本為' + version + '，請升級至1.3.4.3';
        }
      }
      return validObj;
    }
    ...
    onMounted(() => {
      if (!alreadyAddEventListener.value) addEventListener();
      if (!alreadyAddEventListener.value) alreadyAddEventListener.value = !alreadyAddEventListener.value;
    });

    onUpdated(() => {
      const valid = isPIDModelProp || vDataProp.agencyCardType.$model === 'Y';
      // 載入憑證必要資訊
      if (valid) myLoad();
    });
