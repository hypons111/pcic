<template>
  <b-card no-body :class="{ 'border-0': border0 }" v-if="$attrs.items !== undefined || state.itemsUndefinedBehaviorComputed">
    ...
    <b-card-body class="m-0 p-0">
      ...
      <div class="buttonGroup" v-if="addButton || downloadButton || statisticsButton || idkWhatIsThisButton || exportCSVButton">
        <button id="addButton" class="mt-1 ml-1 btn btn-info" v-if="addButton">新增</button>
        <button id="downloadButton" class="mt-1 ml-1 btn btn-info" v-if="downloadButton" >下載</button>
        <button id="statisticsButton" class="mt-1 ml-1 btn btn-info" v-if="statisticsButton">統計</button>
        <button id="idkWhatIsThisButton" class="mt-1 ml-1 btn btn-info" v-if="idkWhatIsThisButton">下載可解析未公布清單</button>
        <button id="exportCSVButton" class="mt-1 ml-1 btn btn-info" v-if="exportCSVButton">匯出 CSV</button>
      </div>
      ...
      <b-table
        ...
        <!-- 序號 -->
        <template #cell(tableNo)="row">{{ state.pagination.start + row.index }}</template>
      </b-table>
    </b-card-body>
    ...
    <div
      ...
      <b-row align-h="around" align-v="center">
        ...
        <b-col lg="3" class="pt-1 pt-lg-0 text-center text-lg-left">
          ...
          {{ state.pagination.totalRows ? state.pagination.totalRows : '沒給totalItems' }} 筆 , 頁次 
