    ...
    <section class="container pt-4">
      <div class="card">
        ...
        <div class="card-body">
          ...
          <b-form-row>
            <!-- 公司名稱 -->
            <i-form-group-check labelStar :label="$t('label.comName') + '：'" :item="$v.name">
              <b-form-input v-model="$v.name.$model" type="text" :state="validateState($v.name)" trim></b-form-input>
            </i-form-group-check>
            <!-- 統一編號 -->
            <i-form-group-check labelStar :label="$t('label.compIdno') + '：'" :item="$v.compIdno">
              <b-form-input v-model="$v.compIdno.$model" :state="validateState($v.compIdno)" @blur="findAdmCompany" trim></b-form-input>
            </i-form-group-check>
          </b-form-row>
          ...
          <b-form-row>
            <!-- 負責人 -->
            <i-form-group-check labelStar :label="$t('label.rpName') + '：'" :item="$v.rpName">
              <b-form-input v-model="$v.rpName.$model" trim type="text" :state="validateState($v.rpName)"></b-form-input>
            </i-form-group-check>
          </b-form-row>

          <b-form-row>
            <!-- 設立日期 -->
            <i-form-group-check labelStar content-cols="5" :label="$t('label.conDate') + '：'" :item="$v.conDate">
              <i-date-picker v-model="$v.conDate.$model" :state="validateState($v.conDate)" placeholder="yyy/MM/dd"></i-date-picker>
            </i-form-group-check>
            <!-- 資本額 -->
            <i-form-group-check :label="$t('label.capital') + '：'" :item="$v.amount">
              <b-form-input v-model="$v.amount.$model" trim :state="validateState($v.amount)"></b-form-input>
            </i-form-group-check>
          </b-form-row>
          ...
          <b-form-row>
            ...
            <i-form-group-check
              ...
              :item="[$v.addressTown, $v.addressRoad, $v.addressCity]"
            >
              <i-address-picker
                :zip.sync="$v.addressZip.$model"
                :city.sync="$v.addressCity.$model"
                :town.sync="$v.addressTown.$model"
                :other.sync="$v.addressRoad.$model"
                trim
                :city-state="validateState($v.addressCity)"
                :town-state="validateState($v.addressTown)"
                :other-state="validateState($v.addressRoad)"
              >
              </i-address-picker>
            </i-form-group-check>
          </b-form-row>
          ...
          <b-form-row>
            <!-- 姓名 -->
            <i-form-group-check labelStar content-cols="5" :label="$t('label.name') + '：'" :item="$v.chName">
              <b-form-input v-model="$v.chName.$model" trim :state="validateState($v.chName)"></b-form-input>
            </i-form-group-check>
            <!-- 身分證字號 -->
          ...
          <b-form-row>
            <!-- 手機 -->
            <i-form-group-check labelStar content-cols="6" :label="$t('label.cellphone') + '：'" :item="$v.mobileTel">
...
export default {
  ...
  setup() {
    ...
    const formDefault = {
      ...
      name: '', // 名稱
      addressZip: '',
      addressCity: '', // 所在地址縣市名稱
      addressTown: '', // 所在地址鄉鎮市區名稱
      addressRoad: '', // 所在地址街道門牌
      amount: '', // 資本額
      conDate: 'null', // 設立日期
      chName: '', // 中文姓名
      idno: '', // 身分證字號
      ...
      workAddrCity: '', // 辦公縣市
      workAddrTown: '', // 辦公鄉政市區
      workAddrOther: '', // 辦公其他地址
      workTel: null, // 室內聯絡電話
      mobileTel: null, // 手機聯絡電話
      email: '', // 電子郵件信箱
      agencyCardType: 'Y',
      smartCard: '', // 信用卡
    ...
    // 表單物件驗證規則
    const rules = {
      compIdno: { required, taxIdNumber: taxIdNumber },
      rpName: { required },
      name: { required },
      addressZip: {},
      addressCity: { required },
      addressTown: { equalsCityBoolean: address(() => form.addressCity) },
      addressRoad: { equalsCityBoolean: address(() => form.addressCity) },
    ...
    // 檢查此統編是否已申請過
    function findAdmCompany(compIdno) {
      if (form.compIdno && !$v.value.compIdno.$error) {
        axios
          .post('/service/get-adm-company-info/' + form.compIdno)
          .then(({ data }) => {
            if (data.status === 'Y') {
              $bvModal.msgBoxOk(`公司已申請管理員帳號（姓名：${data.chName}、連絡電話：${data.workTel}）`);
              form.compIdno = '';
            } else if (data.status === 'N') {
              $bvModal.msgBoxOk('公司已申請管理員帳號停用，請發文至工程會重新啟用管理員帳號');
              form.compIdno = '';
            }
          })
    ...
    function save() {
      axios
        .put('/service/adm-company-contact/save', form)
        .then(({ data }) => {
          reset();
        })
        .finally(() => {})
