<template>
  <div>
      ...
      <div class="card">
        ...
        <div class="card-body col-10">
          <b-collapse v-model="stepVisible">
            ...
            <!-- 逾期日 -->
            <b-form-row>
              <i-form-group-check :label="$t('label.expiredDate')" :item="$v.expiredDate">
                <i-date-picker v-model="$v.expiredDate.$model" :state="$v.expiredDate"></i-date-picker>
              </i-form-group-check>
            </b-form-row>
    ...
    <section>
      <div class="card-body col-10">
        <b-button-toolbar>
          <i-button class="mt-1 ml-10" type="box-arrow-up-right" @click="exportOds"></i-button>
        </b-button-toolbar>
      </div>
    </section>
...
export default {
  ...
  setup() {
    ...
    const handleQuery = () => {
      checkValidity().then((isValid: boolean) => {
        if (isValid) {
          ...
          axios
            .post('/service/rec-projects/expired-finished', form)
            .then(({ data }) => {
    ...
    const exportOds = () => {
      ...
      axios.post('/service/rec-projects/export-ods-expired-finished', form, { responseType: 'blob' }).then(response => {
