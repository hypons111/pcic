  ...
  setup(props, context) {
    ...
    const table = reactive({
      fields: [
        ...
        {
          key: 'businessType',
          label: i18n.t('label.businessType'), //業務別
          sortable: false,
          thClass: 'text-center',
          tdClass: 'text-center align-middle',
        },
    ...
    const handleQuery = () => {
      ...
      form.unitId = userIdentity.value.id;
      const url = '/pwb-unit-apply/query-module';
          ...
          $bvModal.msgBoxConfirm('是否申請').then((isOK: boolean) => {
            if (isOK) {
              ...
              if (item.id !== null) {
                ...
                item.unitId = userIdentity.value.id;
                item.applyTime = new Date();
    ...
    const putForm = (item: any) => {
      let id = item.id;
      let url = '/pwb-unit-applies-update/' + id;
      dataPromise.value = axios
        .put(url, item)
        .then((response: AxiosResponse) => {
          handleQuery();
