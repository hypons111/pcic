...
export default {
  ...
  setup(props, context) {
    ...
    //轉到新增頁面
    const toCreateView = () => {
      navigateByNameAndParams('rev0302EditInfo', { formStatusForward: 'create' });
    };

    //轉到編輯頁面
    const toEditView = (revMain: RevMain) => {
      navigateByNameAndParams('rev0302EditInfo', { formStatusForward: 'modify', revMain: revMain, isNotKeepAlive: true });
    };

    //轉到檢視頁面
    const toInfoView = ((revMain: RevMain) => {
      navigateByNameAndParams('rev0302EditInfo', { formStatusForward: 'info', revMain: revMain, isNotKeepAlive: true });
    };
