...
import Eng0106ServiceCaseDetail from '@/components/eng/eng0106/eng0106-service-case-detail.vue';
import _ from 'lodash';

export default {
  name: 'Eng0106ServiceCase',
  props: {
    engYearreport: {
      type: Object,
      required: false,
  ...
  components: {
    Eng0106ServiceCaseDetail,
  },
  setup(props) {
    const eng0106ServiceCaseDetail = ref(null);
    ...
    const handleOk = (bvModalEvt: BvModalEvent) => {
      ...
      eng0106ServiceCaseDetail.value.checkValidity().then((isValid: boolean) => {
        if (isValid) {
          const data = eng0106ServiceCaseDetail.value.sendData();
    ...
    const modalReset = () => {
      eng0106ServiceCaseDetail.value.modalReset();
    };
