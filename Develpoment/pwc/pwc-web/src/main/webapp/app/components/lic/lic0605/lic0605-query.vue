    ...
    <section class="container mt-3" v-if="queryStatus">
      ...
      >
        ...
        <template #cell(action)="row">
          <b-button class="mt-1" variant="info" @click="toLic0605HIstoryView(row.item)">歷次辦理情形</b-button>
        </template>
      </i-table>
    </section>
  </div>
...
export default {
  ...
  setup(props, context) {
    ...
    //轉到歷次辦理情形頁面
    const toLic0605HIstoryView = (licLicense: any) => {
      navigateByNameAndParams('lic0605HistoryInfo', {
        licLicense: licLicense,
        isNotKeepAlive: true,
      });
    };
