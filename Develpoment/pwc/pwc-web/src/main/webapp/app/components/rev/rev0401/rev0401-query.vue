...
export default {
  ...
  setup(props, context) {
    ...
    //轉到新增頁面
    const toCreateView = () => {
      navigateByNameAndParams('rev0402EditInfo', { formStatusForward: 'create' });
    };

    //轉到編輯頁面
    const toEditView = (revMain: any) => {
      navigateByNameAndParams('rev0402EditInfo', { formStatusForward: 'modify', revMain: revMain, isNotKeepAlive: true });
    };

    //轉到檢視頁面
    const toInfoView = (revMain: any) => {
      navigateByNameAndParams('rev0402EditInfo', { formStatusForward: 'info', revMain: revMain, isNotKeepAlive: true });
    };

    //轉到審查頁面
    const toReviewView = (revMain: any) => {
      navigateByNameAndParams('rev0403ReviewInfo', { revMain: revMain, isNotKeepAlive: true });
    };
    ...
    const deleteData = (deleteId: string) => {
      ...
      //   }
      // });
      $bvModal.msgBoxConfirm('確定刪除基本設計資料？').then((isOK: boolean) => {
