<template>
  ...
  >
    ...
    <div v-else-if="itemProp != null">
      <b-form-invalid-feedback :force-show="true" v-if="itemProp.$errors.length > 0">
...
export default defineComponent({
  ...
  setup(props, context) {
    const itemProp = props.item != null ? toRef(props, 'item') : ref(null);
    ...
    watch(
      dual1Prop,
      () => {
        // console.log('dual1Prop',dual1Prop);
        dualCheck();
      },
      { deep: true }
    );
    watch(
      dual2Prop,
      () => {
        // console.log('dual2Prop',dual2Prop);
        dualCheck();
      },
      { deep: true }
    );
    ...
    function marginCheckFunction(): void {
      if (itemProp.value.$error) marginCheck.value = true;
      else if (itemProp.value.$error === false) marginCheck.value = false;
      // console.log(itemProp);
      // console.log(labelProp.value);
      // console.log(marginCheck.value);
    }
