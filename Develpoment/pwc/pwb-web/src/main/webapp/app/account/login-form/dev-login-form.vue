        ...
        <div class="card-body">
          <b-form role="form" v-on:submit.prevent="doLogin()">
            <b-form-group label="使用者帳號" label-for="id" description="
                機關帳號：工程會自訂機關代碼簡碼+ '-' +流水號，例如：9.99-0。 若機關為工程會，則帳號為 工程會機關代碼(3.60)+ '.' + 單位代碼(1~15) + '-' +流水號，例如：3.60.1-6。
                公司法人帳號：公司統編 + '- + 流水號。
                個人帳號：限英數字，禁用.跟-。">
              <b-form-input id="id" type="text" name="id" autofocus v-model="form.id" data-cy="id"></b-form-input>
            </b-form-group>
            ...
            <b-form-group>
              <i-tree-view ref="menuTree" @selectNode="selectFunction($event)" :model="data"></i-tree-view>
  ...
  setup() {
    ...
    function doLogin() {
      ...
      axios.post('account', { ...form, authorities: authorities }).then(() =>
        axios.post('authenticate', { username: form.id, password: 'REMOVE_ME' }).then(result => {
