...
public class RecAreaRepositoryImpl implements RecAreaRepositoryCustom {
    ...
    public RecAreaRepositoryImpl(SqlExecutor sqlExecutor, SerialNumGenerator serialNumGenerator) {
    ...
    @Override
    public Page<RecAreaDTO> findAllByCriteria(RecAreaCriteria criteria) {
        Query queryTotalCount = Query
                .builder()
                .append("SELECT COUNT(*) FROM REC_AREA WHERE 1=1")
        ...
        Query query = Query
                .builder()
                .append("SELECT ROW_NUMBER() OVER (ORDER BY "+ criteria.getOrderBy()+ ") AS row_num, * FROM REC_AREA WHERE 1=1")
                .appendWhen(StringUtils.isNotBlank(criteria.getProjectNo()),"AND PROJECT_NO LIKE :projectNo","%"+criteria.getProjectNo()+"%" )
                .appendWhen(StringUtils.isNotBlank(criteria.getAreaNo()),"AND AREA_NO LIKE :areaNo","%"+criteria.getAreaNo()+"%"  )
                ...
                .build();
        List<RecAreaDTO> recAreaDTOS = sqlExecutor.queryForList(query, RecAreaDTO.class);
        Page<RecAreaDTO> resultPage = new PageImpl<>(recAreaDTOS,criteria.toPageable(),totalCount);
        return resultPage;
    }

    @Override
    public RecAreaDTO packPkAndLogColumn(RecAreaDTO recAreaDTO) throws TypeVariableNotMatchException {
        String arePk = serialNumGenerator
                .setModule(ModuleEnum.REC)
                .setVariable(new SerialNumVariableParam(RecGenNumberVariableEnum.Are.getVariable()))
                .getGeneratedNumber();
        recAreaDTO.setAreaNo(arePk);
        return recAreaDTO;
    }
}
