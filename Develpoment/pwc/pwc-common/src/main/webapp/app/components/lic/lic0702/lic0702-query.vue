      ...
      <div class="container" v-if="queryStatus">
        ...
        >
          <template #cell(action)="row">
            ...
            <i-button class="mt-1" type="trash" @click="deleteProject(row.item.licWorkshopMeetingId)"></i-button>
          </template>
        </i-table>
      </div>
    </section>
...
export default {
  ...
  setup() {
    ...
    //轉到新增頁面
    const toCreateView = () => {
      navigateByNameAndParams('lic0702-edit-info', { formStatusForward: 'create' , isNotKeepAlive: true  });
    };

    //轉到編輯頁面
    const toEditView = (licWorkshopMeeting: any) => {
      navigateByNameAndParams('lic0702-edit-info', { formStatusForward: 'modify', licWorkshopMeeting: licWorkshopMeeting, isNotKeepAlive: true });
    };
