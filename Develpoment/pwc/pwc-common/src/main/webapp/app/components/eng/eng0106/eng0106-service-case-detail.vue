<template>
  <b-container>
    ...
    <b-form-row>
      <!-- 案主地址 -->
      <i-form-group-check :label="$t('label.clientAddr') + '：'" class="col-12" label-cols="2" content-cols="10">
        <i-address-picker
          :zip.sync="$v.clientAddrZip.$model"
          :city.sync="$v.clientAddrCity.$model"
          :town.sync="$v.clientAddrTown.$mode"
          :other.sync="$v.clientAddrOther.$model"
          :cityState="validateState($v.clientAddrCity)"
          :townState="validateState($v.clientAddrTown)"
          :otherState="validateState($v.clientAddrOther)"
        ></i-address-picker>
      </i-form-group-check>
    </b-form-row>
    ...
    <b-form-row>
      <!-- 備註 -->
      <i-form-group-check :label="$t('label.remark') + '：'" :item="$v.remark" class="col-12" label-cols="2" content-cols="10">
    ...
    >
      <template #table-top>
        <b-row class="mb-2">
          <b-col align="end">
            請選擇參與本案的技師名單：
            <b-form-select v-model="selectedEngr" :options="engYearreportEngr" value-field="engrIdno" text-field="engrName" class="col-2">
              <template #first>
                <option value="">-- 請選擇 --</option>
              </template>
            </b-form-select>
      ...
      <template #cell(subject)="row">
        <b-form-select v-model="row.item.subject" :options="filterSubject(row.item.subjectList)">
          <template #first>
            <option value="">-- 請選擇 --</option>
          </template>
        </b-form-select>
      ...
      <template #cell(mainTask)="row">
        <b-form-textarea v-model="row.item.mainTask" rows="3"></b-form-textarea>
      </template>
...
export default {
  ...
  setup(props) {
    ...
    function getClientAddrZip(param: string) {
      form.clientAddrZip = param;
    }
    function getClientAddrCity(param: string) {
      form.clientAddrCity = param;
    }
    function getClientAddrTown(param: string) {
      form.clientAddrTown = param;
    }
    function getClientAddrOther(param: string) {
      form.clientAddrOther = param;
    }
    ...
    const deleteEngr = (engYearreportCaseEngrId: string) => {
      engYearreportCaseEngrRef.value = engYearreportCaseEngrRef.value.filter(
        item => item.engYearreportCaseEngrId !== engYearreportCaseEngrId
      );
