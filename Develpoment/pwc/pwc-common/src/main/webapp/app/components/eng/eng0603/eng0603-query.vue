  ...
  <div>
          ...
          <b-collapse v-model="stepVisible.step1">
            <b-form-row>
              ...
              <i-form-group-check :label="$t('label.engName') + '：'" :item="$v.engName">
                <b-form-input v-model="$v.engName.$model" trim></b-form-input>
              </i-form-group-check>
            </b-form-row>
    ...
    <section class="container mt-2" v-show="queryStatus">
      ...
      >
        ...
        <template #cell(idno)="{ item }">
          <a class="href-style" href="javascript: void(0)" @click="toEditView(item, FormStatusEnum.READONLY, item)">{{ item.idno }}</a>
        </template>
      </i-table>
    </section>
  </div>
...
export default {
  ...
  setup(props) {
    ...
    const toEditView = (entity: IEng0601, formStatus: FormStatusEnum, index?: number) => {
      ...
      navigateByNameAndParams('Eng0603EditInfo', {
