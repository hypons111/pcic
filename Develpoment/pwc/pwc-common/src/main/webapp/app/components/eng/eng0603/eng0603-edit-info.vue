      ...
      <div class="card">
        ...
        <div class="card-body">
          <b-form-row v-if="formStatusRef === FormStatusEnum.READONLY || formStatusRef === FormStatusEnum.EDIT">
            ...
            <!-- 證書字號 -->
            <i-form-group-check content-cols="4" :label="$t('label.engrCertificateDocNo') + '：'" :item="$v.engrCertificateDocNo">
              <!-- <div v-if="formStatusRef === FormStatusEnum.READONLY">{{ $v.engrCertificateDocNo.$model }}</div>
              <b-form-input v-else v-model="$v.engrCertificateDocNo.$model" :state="validateState($v.engrCertificateDocNo)" trim lazy> </b-form-input> -->
              <div v-if="formStatusRef === FormStatusEnum.READONLY">{{ data.engrCertificateDocNo }}</div>
              <b-form-input v-else v-model="data.engrCertificateDocNo" :state="validateState($v.engrCertificateDocNo)" trim lazy> </b-form-input>
            </i-form-group-check>
          </b-form-row>
          ...
          <b-form-row v-if="formStatusRef === FormStatusEnum.READONLY || formStatusRef === FormStatusEnum.EDIT || formStatusRef === FormStatusEnum.CREATE">
            ...
            <!-- 開業證號 -->
            <i-form-group-check content-cols="4" :label="$t('label.businesscertificatenum') + '：'" :item="$v.businesscertificatenum">
              <!-- <div v-if="formStatusRef === FormStatusEnum.READONLY">{{ $v.businesscertificatenum.$model }}</div>
              <b-form-input v-else v-model="$v.businesscertificatenum.$model" :state="validateState($v.businesscertificatenum)" trim lazy> </b-form-input> -->
              <div v-if="formStatusRef === FormStatusEnum.READONLY">{{ data.businesscertificatenum }}</div>
              <b-form-input v-else v-model="data.businesscertificatenum" :state="validateState($v.businesscertificatenum)" trim lazy> </b-form-input>
            </i-form-group-check>
          </b-form-row>

          <b-form-row v-if="formStatusRef === FormStatusEnum.READONLY || formStatusRef === FormStatusEnum.EDIT">
            <!-- 近3年擔任主任建築師年資 -->
            <i-form-group-check content-cols="4" :label="$t('label.arch3iSen') + '：'" :item="$v.arch3iSen">
              <!-- <div v-if="formStatusRef === FormStatusEnum.READONLY">{{ $v.arch3iSen.$model }}</div>
              <b-form-input v-else v-model="$v.arch3iSen.$model" :state="validateState($v.arch3iSen)" trim lazy> </b-form-input> -->
              <div v-if="formStatusRef === FormStatusEnum.READONLY">{{ data.arch3iSen }}</div>
              <b-form-input v-else v-model="data.arch3iSen" :state="validateState($v.arch3iSen)" trim lazy> </b-form-input>
            </i-form-group-check>
          </b-form-row>
        </div>
      </div>
  ...
  <b-table-simple hover small caption-top responsive>
    <b-thead head-variant="dark">
      <b-tr>
        ...
        <b-th class="text-center" colspan="4">公共工程</b-th>
      </b-tr>
      <b-tr>
        <b-th>近3年</b-th>
        <b-th>2億以上</b-th>
    ...
    <div class="text-center mb-2">
      ...
      <a class="href-style" href="javascript: void(0)" v-b-modal.query-0603-modal>明細</a>
    </div>
...
export default defineComponent<propsType>({
  ...
  setup(props) {
    ...
    const toEditView = (entity: IEngEngrSubjectpoint, formStatus: FormStatusEnum, index?: number) => {
        ...
        navigateByNameAndParams('Eng0603Query', {
