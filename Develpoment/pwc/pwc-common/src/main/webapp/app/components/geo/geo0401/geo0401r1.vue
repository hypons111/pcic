      ...
      <div class="card">
        ...
        <div class="card-body">
          <b-collapse v-model="stepVisible.step1">
            <!-- <button @click="handleRemark">查remark</button> -->
            <b-form-group class="col-12" label-cols="0" content-cols="0" label="" label-align="right">
              <b-form-textarea v-if="form.remark" v-model="$v.remark.$model" readonly></b-form-textarea>
            </b-form-group>
            <b-form-row>
              <i-form-group-check class="col-6" label-cols="4" content-cols="8" :label="$t('label.questionTime') + '：'">
                <b-input-group>
                  <i-date-picker
              ...
              <i-form-group-check class="col-6" label-cols="4" content-cols="8" :label="$t('label.questionKeyWord') + '：'">
    ...
    <section class="container mt-3" v-if="queryStatus">
      ...
      >
        <template #cell(action)="row">
          <i-button class="mb-1" size="sm" type="pencil-square" @click="toEditView(row.item)"></i-button>
          <i-button class="mb-1" size="sm" type="trash" @click="handleDelete(row.item)"></i-button>
        </template>
      </i-table>
    </section>
  </div>
...
import { computed, onActivated, reactive, Ref, ref, toRefs, watch, onMounted } from '@vue/composition-api';
...
export default {
  ...
  setup(props, context) {
    ...
    const handleDelete = (geoQuestion: GeoQuestion) => {
      $bvModal.msgBoxConfirm('確定刪除此筆問題?').then((isOK: boolean) => {
        if (isOK) {
          putForm(geoQuestion);
    ...
    const putForm = (geoQuestion: GeoQuestion) => {
      let url: string;
      url = `/geo-questions/${geoQuestion.id}`;
      geoQuestion.isDel = 'Y';
      geoQuestion.delDate = new Date();
      geoQuestion.delAccId = geoQuestion.geoQuestionId;
      dataPromise.value = axios
        .put(url, geoQuestion)
        .then(({ data }) => {
          clickQueryBtn();
    ...
    const openAddView = (geoQuestion: GeoQuestion, formStatus: Ref<FormStatusEnum>) => {
      navigateByNameAndParams('geo0401Add', { geoQuestion: geoQuestion, formStatus: formStatus });
    };
    const toEditView = (geoQuestion: GeoQuestion) => {
      navigateByNameAndParams('geo0401Edit', { geoQuestion: geoQuestion });
    };

    function notBeforeCreateTimeStart(date: Date) {
      if (form.createTimeStart) return date < new Date(form.createTimeStart);
    }

    function notAfterCreateTimeEnd(date: Date) {
      if (form.createTimeEnd) return date > new Date(form.createTimeEnd);
    }
    ...
    const handleRemark = () => {
      account.role = '0' + account.role;
      dataPromise.value = axios
        .post('/geo-questions/criteria-remark', account)
        .then(({ data }) => {
          // console.log(data);
          formDefault.remark = data[0].value;
          form.remark = data[0].value;
          isReload.value = true;
