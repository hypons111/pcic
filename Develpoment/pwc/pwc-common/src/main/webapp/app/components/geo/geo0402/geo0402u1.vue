    ...
    <section class="container">
      <div class="card">
        ...
        <div class="card-body">
          ...
          <b-form-row class="align-items-center mt-2">
            <i-form-group-check class="col-12" label-cols="2" content-cols="9" :label="$t('label.reply') + '：'" :item="$v.question">
              <b-form-textarea cols="50" rows="5" v-model.trim="$v.reply.$model" :state="validateState($v.reply)"></b-form-textarea>
            </i-form-group-check>
          </b-form-row>
...
import { onActivated, reactive, Ref, ref, toRefs, watch } from '@vue/composition-api';
...
export default {
  ...
  setup(props, context) {
    ...
    const rules = {
      id: {},
      question: { notNull: required, maxLength: maxLength(500) },
      reply: { notNull: required, maxLength: maxLength(500) },
      updateUser: {},
      updateTime: {},
      status: {},
    };
    ...
    const toQueryView = () => {
      context.emit('changeComponent', 'geo0402Query', isReload.value);
    };

    const submitForm = () => {
      checkValidity().then((isValid: boolean) => {
        if (isValid) {
          $bvModal.msgBoxConfirm('確定回答會員此問題之內容？').then((isOK: boolean) => {
            if (isOK) {
              putForm();
            }
          });
