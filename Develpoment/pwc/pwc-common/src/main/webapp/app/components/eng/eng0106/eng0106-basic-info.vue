<template>
  <b-container>
    ...
    <b-form-row>
      ...
      <!-- 執業方式 -->
      <i-form-group-check :label="$t('label.execType') + '：'" :item="$v.execType" content-cols="4">
        <div>{{ formOptionsFormatter(formOptions.execType, $v.execType.$model) }}</div>
      </i-form-group-check>
    </b-form-row>
    ...
    <b-form-row>
      ...
      <!-- 技師科別 -->
      <i-form-group-check :label="$t('label.engrSubject') + '：'" :item="$v.engrSubjectList">
        <b-form-input plaintext disabled :value="$v.engrSubjectList.$model"></b-form-input>
      </i-form-group-check>
    </b-form-row>

    <b-card sub-title="事務所收入（本欄位為配合主計處服務業統計資料用，不作其他用途）" sub-title-text-variant="secondary" class="mb-4">
      <hr />
      <b-card-text class="mb-3">
        <!-- 總收入 -->
        {{ i18n.t('label.incomeTotal') }} <span id="incomeTotal"> {{ $v.incomeTotal.$model | commaFormatter }} </span> 元
      </b-card-text>
      <b-popover
        v-if="$v.engYearreportAuth.isAuthorize.$model !== '1'"
        placement="topright"
        triggers="hover"
        variant="success"
        target="incomeTotal"
      >
        <div class="font-weight-bold">
          本系統已新增向財政部申請查調貴公司申報所得功能，您可以透過『授權服務書』分頁，勾選同意並授予本會權限向財政部查調台端所提報之個人所得稅『年度總所得』及『執行業務所得』。
        </div>
      </b-popover>
      <b-card-text>
        <!-- 營業收入 -->
        ...
        <!-- 技術服務費 -->
        元，為本年度{{ i18n.t('label.incomeTechService') }}完成金額總計
        <b-form-input
          ...
          v-i-currency
        ></b-form-input>
        元（包含國內 {{ $v.incomeTechServiceDomestic.$model | commaFormatter }} 元 + 國外
        {{ $v.incomeTechServiceForeign.$model | commaFormatter }} 元）
        <br />
        <span class="remark-text"
          >（「本年度技術服務費完成金額」可參考技師個人綜合所得申報書表之「年度執行業務者收入明細表」與技師執業範圍有關項目收入加總）</span
        >
        +
        <!-- 非技術服務費 -->
        <div class="py-1">
          {{ i18n.t('label.incomeNonTechService') }} {{ $v.incomeNonTechService.$model | commaFormatter }} 元
          <span class="remark-text">（「年度執行業務者收入明細表」以其他執行業務身分之收入則屬「非技術服務收入」）</span>
        </div>
      ...
      <b-card-text>
        <table>
          ...
          <tr>
            <th valign="top">3.</th>
            <td>
              非營業收入 = 租金收入 + 利息收入 + 其他非營業收入<span class="remark-text"
                >（註：以利用事務所或技師身分取得之非營業收入為限 [ 如參與審查或查核等收入 ] ）</span
              >
            </td>
          </tr>
        </table>
    ...
    <b-form-row>
      <!-- 國外員工人數 -->
      <i-form-group-check :labelStar="true" :label="$t('label.staffNumForeign') + '：'" :item="$v.staffNumForeign" content-cols="4">
        <b-form-input
    ...
    <b-form-row>
      <!-- 貸款或融資案件數 -->
      <i-form-group-check :labelStar="true" :label="$t('label.countFinancing') + '：'" :item="$v.countFinancing" content-cols="4">
        <b-form-input
      ...
      <i-form-group-check :labelStar="true" :label="$t('label.feeFinancing') + '：'" :item="$v.feeFinancing" content-cols="4">
        <b-form-input
          v-model.trim="$v.feeFinancing.$model"
          :state="validateState($v.feeFinancing)"
          placeholder="請輸入正整數"
          v-i-currency
        ></b-form-input>
      </i-form-group-check>
