          ...
          <b-collapse v-model="stepVisible.step1">
            <b-form-row>
              ...
              <i-form-group-check
                ...
                label-cols="4"
                content-cols="8"
                :label="$t('label.jobKeyWord') + '：'"
                :item="$v.jobKeyWord"
              >
                <b-form-input v-model="$v.jobKeyWord.$model"> </b-form-input>
              </i-form-group-check>
            </b-form-row>
            <b-form-row>
              <b-col offset="8">
    ...
    <section class="container mt-3" v-if="queryStatus">
      ...
      >
        <template #cell(action)="row">
          <!-- <i-button size="sm" type="clipboard-plus" @click="toEditView(row.item)"></i-button> -->
          <b-button size="sm" @click="toEditView(row.item)">審核</b-button>
        </template>
      </i-table>
    </section>
  </div>
