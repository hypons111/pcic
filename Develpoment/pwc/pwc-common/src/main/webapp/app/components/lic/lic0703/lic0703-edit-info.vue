            ...
            <div>
              <table border="2" cellpadding="2" width="100%">
                ...
                <tr v-for="(item,key) in table1.data">
                  ...
                  <td style="vertical-align : middle">{{item.licPlatformLicenseId}}</td>
                  <td>
...
export default {
  ...
  setup(props, context) {
    ...
    table1.data = [
      {
        licPlatformLicenseId: '大地技師證照',
      },
      {
        licPlatformLicenseId: '環工證照',
      },
      {
        licPlatformLicenseId: '水利證照',
      },
      {
        licPlatformLicenseId: '工地安全證照',
      },
    ];
    ...
    //刪除推動平臺成員檔
    const deleteMember = () => {
      $bvModal.msgBoxConfirm('刪除推動平臺成員檔（單筆），確定刪除？').then((isOK: boolean) => {
        if (isOK) {
          let url: string;
          url = `/rec-typhoons/${form.typhoonNo}`;
          axios
    ...
    //刪除推動平臺成員證照檔
    const deleteLicense = () => {
      $bvModal.msgBoxConfirm('刪除推動平臺成員證照檔（單筆），確定刪除？').then((isOK: boolean) => {
        if (isOK) {
          let url: string;
          url = `/rec-typhoons/${form.typhoonNo}`;
          axios
