    ...
    <div class="card" id="printTable">
      <div class="card-body col-12">
        <section class="container mt-2">
          ...
          <div class="d-flex justify-content-center"><h5>工程停工報告表</h5></div>
        ...
        <section class="container mt-2">
          <b-table-simple bordered fixed>
            ...
            <!-- 開始 復工 停工 -->
            <b-tr>
              <b-td colspan="4"> 本工程定於中華民國 {{ formatDateInChinese(new Date(bidProjectStopwk.startDate)) }}停工</b-td>
            </b-tr>
            <!-- 承包商 契約規定工期 -->
            <b-tr>
            ...
            <!-- 說明 -->
            <b-tr>
              <b-td class="text-center">說明</b-td>
              <b-td colspan="3">{{ bidProjectStopwk.remark }}</b-td>
            </b-tr>
            <!-- 建議協調層級 -->
            <b-tr>
              <b-td class="text-center">建議協調層級</b-td>
              <b-td colspan="3">{{ bidProjectStopwk.negoLevel }}</b-td>
            </b-tr>
            <!-- 待協調事項涉及機關名稱 -->
            <b-tr>
              <b-td class="text-center">待協調事項涉及機關名稱</b-td>
              <b-td colspan="3">{{ bidProjectStopwk.negoUnit }}</b-td>
            </b-tr>
            <!-- 提報單位 監造單位 主辦機關 -->
            <b-tr>
              <b-td colspan="2">
  ...
  setup(props, context) {
    ...
    function fetchStopwkInfo(wkut: string, prjno: string, startDate: Date) {
      bidPrjStopwkService.getBidProjectStopwkByKey(wkut, prjno, startDate).then(data => {
        ...
        processStopwkData(bidProjectStopwk.value);
      });
    }

    //format用
    function processStopwkData(stopwk: IBidPrjStopwk) {
      bidSysCodeTable.getBidSysCodeWithParam('BID_012').then(data => {
        let negoLevelKey = stopwk.negoLevel;
        let finObj = data.find(result => {
          return result.dataKey === negoLevelKey;
        });
        stopwk.negoLevel = finObj.value;
      });
    }
