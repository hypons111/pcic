  ...
  <b-card no-body>
    ...
    <b-card-body>
      <b-tabs content-class="mt-4">
        <b-tab title="執業機構基本資料" :active="activeTab(0)" @click="changeTabIndex(0)" :disabled="tabDisabled[0]">
          <eng-0106-basic-info :engYearreport="engYearreportRef" ref="eng0106BasicInfo" />
        </b-tab>
        <!-- <b-tab title="執業技師人員名冊" :active="activeTab(2)" @click="changeTabIndex(2)" :disabled="tabDisabled[2]">
          <eng-0106-engr-roster :engYearreport="engYearreportRef" ref="eng0106EngrRoster"/>
        </b-tab>
        <b-tab title="年度異動核備情形" :active="activeTab(3)" @click="changeTabIndex(3)" :disabled="tabDisabled[3]">
          <eng-0106-change-situation :engYearreport="engYearreportRef" ref="eng0106ChangeSituation" />
        </b-tab> -->
        <b-tab title="年度服務案件統計" :active="activeTab(1)" @click="changeTabIndex(1)" :disabled="tabDisabled[1]">
          <eng-0106-service-case :engYearreport="engYearreportRef" ref="eng0106ServiceCase" />
        </b-tab>
        <b-tab title="執業機構員工統計" :active="activeTab(2)" @click="changeTabIndex(2)" :disabled="tabDisabled[2]">
          <eng-0106-questionnaire :engYearreport="engYearreportRef" ref="eng0106Questionnaire" />
        </b-tab>
        <b-tab title="授權服務書" :active="activeTab(3)" @click="changeTabIndex(3)" :disabled="tabDisabled[3]">
          <eng-0106-authorization-service :engYearreport="engYearreportRef" ref="eng0106AuthorizationService" />
        </b-tab>
      ...
      <div class="text-center mt-2">
        <i-button v-show="true" type="send-check" @click="submitForm(true)"></i-button>
        <i-button v-show="true" type="save" @click="submitForm(false)"></i-button>
...
import Eng0106BasicInfo from '@/components/eng/eng0106/eng0106-basic-info.vue';
import Eng0106EngrRoster from '@/components/eng/eng0106/eng0106-engr-roster.vue';
import Eng0106ChangeSituation from '@/components/eng/eng0106/eng0106-change-situation.vue';
import Eng0106ServiceCase from '@/components/eng/eng0106/eng0106-service-case.vue';
import Eng0106Questionnaire from '@/components/eng/eng0106/eng0106-questionnaire.vue';
import Eng0106AuthorizationService from '@/components/eng/eng0106/eng0106-authorization-service.vue';

export default {
  name: 'Eng0106EditInfo',
  props: {
    engYearreport: {
      type: Object,
      required: false,
  ...
  components: {
    ...
    Eng0106BasicInfo,
    Eng0106EngrRoster,
    Eng0106ChangeSituation,
    Eng0106ServiceCase,
    Eng0106Questionnaire,
    Eng0106AuthorizationService,
  },
  ...
  setup(props) {
    ...
    const eng0106BasicInfo = ref(null);
    const eng0106EngrRoster = ref(null);
    const eng0106ChangeSituation = ref(null);
    const eng0106ServiceCase = ref(null);
    const eng0106Questionnaire = ref(null);
    const eng0106AuthorizationService = ref(null);
    ...
    const nextTab = () => {
      ...
      //     break;
      //   case 1:
      //     eng0106BasicInfo.value.checkValidity().then((isValid: boolean) => {
      //       if (isValid) {
      //         tabDisabled[2] = false;
      //         tabIndex.value++;
      //       } else {
    ...
    const reset = () => {
      switch (tabIndex.value) {
        case 0:
          break;
        case 1:
          eng0106BasicInfo.value.reset();
          break;
        case 2:
          eng0106EngrRoster.value.reset();
          break;
        case 3:
          break;
        case 4:
          eng0106ServiceCase.value.reset();
          break;
        case 5:
          eng0106Questionnaire.value.reset();
          break;
        case 6:
    ...
    const submitForm = (isCheck: boolean = false) => {
      if (isCheck) {
        Promise.all([eng0106BasicInfo.value.checkValidity(), 2, 3, 4, eng0106Questionnaire.value.checkValidity()]).then(
          (res: Array<boolean>) => {
    ...
    const putForm = () => {
      ...
      // Object.assign(form, eng0106BasicInfo.value.putForm());
      // form['engYearreportEngr'] = Object.assign([], eng0106EngrRoster.value.putForm());

      Object.assign(engYearreportRef.value['engYearreportAuth'], eng0106AuthorizationService.value.putForm());
      Object.assign(engYearreportRef.value, eng0106BasicInfo.value.putForm());
      // engYearreportEngr陣列長度不會改變, 只能變更單筆資料的備註, 所以可以用Object.assign覆蓋原陣列。
      Object.assign(engYearreportRef.value['engYearreportEngr'], eng0106EngrRoster.value.putForm());
      Object.assign(engYearreportRef.value, eng0106Questionnaire.value.putForm());
      // engYearreportCase陣列長度可能會改變, 所以不能直接用Object.assign, 例如長度3 -> 2, 只會覆蓋原陣列前兩筆。
      // 先指定為空陣列
      engYearreportRef.value['engYearreportCase'] = [];
      Object.assign(engYearreportRef.value['engYearreportCase'], eng0106ServiceCase.value.putForm());
      ...
      //   ...eng0106ServiceCase.value.putForm()
      // );
    };
