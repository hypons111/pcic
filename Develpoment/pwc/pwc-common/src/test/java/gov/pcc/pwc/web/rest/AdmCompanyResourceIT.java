...
class AdmCompanyResourceIT {
    ...
    private static final String ENTITY_API_URL_ID = ENTITY_API_URL + "/{ban}";

    @Autowired
    private AdmCompanyRepository admCompanyRepository;

    @Autowired
    private AdmCompanyMapper admCompanyMapper;
    ...
    void createAdmCompany() throws Exception {
        ...
        AdmCompanyDTO admCompanyDTO = admCompanyMapper.toDto(admCompany);
        restAdmCompanyMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(admCompanyDTO)))
...
//        admCompany.setBan(UUID.randomUUID().toString());
        admCompany.setCompIdno(UUID.randomUUID().toString());
        admCompanyRepository.saveAndFlush(admCompany);

        // Get all the admCompanyList
        restAdmCompanyMockMvc
            .perform(get(ENTITY_API_URL + "?sort=ban,desc"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
//            .andExpect(jsonPath("$.[*].ban").value(hasItem(admCompany.getBan())))
...
//        admCompany.setBan(UUID.randomUUID().toString());
        ...
        // Get the admCompany
        restAdmCompanyMockMvc
//            .perform(get(ENTITY_API_URL_ID, admCompany.getBan()))
            .perform(get(ENTITY_API_URL_ID, admCompany.getCompIdno()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
//            .andExpect(jsonPath("$.ban").value(admCompany.getBan()))
            .andExpect(jsonPath("$.ban").value(admCompany.getCompIdno()))
...
//        AdmCompany updatedAdmCompany = admCompanyRepository.findById(admCompany.getBan()).get();
        ...
        updatedAdmCompany
            ...
            .updateUser(UPDATED_UPDATE_USER)
            .updateTime(UPDATED_UPDATE_TIME);
        AdmCompanyDTO admCompanyDTO = admCompanyMapper.toDto(updatedAdmCompany);

        restAdmCompanyMockMvc
            .perform(
                put(ENTITY_API_URL_ID, admCompanyDTO.getCompIdno())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(admCompanyDTO))
            )
            .andExpect(status().isOk());
...
//        admCompany.setBan(UUID.randomUUID().toString());
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAdmCompanyMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(admCompanyDTO)))
...
//        partialUpdatedAdmCompany.setBan(admCompany.getBan());
        ...
        restAdmCompanyMockMvc
            .perform(
//                patch(ENTITY_API_URL_ID, partialUpdatedAdmCompany.getBan())
...
//        admCompany.setBan(UUID.randomUUID().toString());
        ...
        restAdmCompanyMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, admCompanyDTO.getCompIdno())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(admCompanyDTO))
            )
            .andExpect(status().isBadRequest());
...
//        admCompany.setBan(UUID.randomUUID().toString());
        ...
        restAdmCompanyMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(admCompanyDTO))
            )
            .andExpect(status().isMethodNotAllowed());
...
//        admCompany.setBan(UUID.randomUUID().toString());
        ...
        restAdmCompanyMockMvc
//            .perform(delete(ENTITY_API_URL_ID, admCompany.getBan()).accept(MediaType.APPLICATION_JSON))
